...
def main():
    ...
    output = {
        ...
        "result": results,
        "input_param_meaning": config.get("input_param_meaning", "value of N (we use NxN matrices)"),
        "test_name": config.get("test_name", "Matrix Multiplication")
    }

    # Get output file path from config or use default
    output_file = config.get("output_file_path", "profile_results.json")
